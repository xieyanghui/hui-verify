...
(function(factory){
    ...
    // 验证是否引用jquery
    if (!$ || !$.fn || !$.fn.jquery) {
        console.log('请引用jQuery');
        return;
    }
    /**
     * 验证单个input
     * @param eem  Element  需要验证的元素
     * @param location Element 消息显示的位置
     * @param name String  可选参数，表单提示显示的名字
     * @param megs object  可选参数，消息集合 可自定义，可直接输出，也可预定义
     * */
...
//输出消息
        output: function (b,meg) {
            ...
            if (b) { //验证成功
                ...
                if(form.length >0 && form.attr('ver_err').indexOf(this.eem.attr('name')) ===-1){
...
//根据消息KEY获取消息
    ...
    /**
     * 执行方法
     * @param el Element需要验证的元素
     * @param verify  object 验证参数集合
     * {
     *
     * name string没有自定义错误消息时需要提示的名称
     *
     * method string触发事件
     *
     * check array 需要验证的方法集合
     *
     * location Element  消息显示的位置
     *
     *  megs ：可选参数，消息集合,key为验证的方法名， 可自定义，可直接输出，也可预定义
     * }
     *
     * */
    var run = function (el, verify) {
        if (!verify) return;
        //信息提示位置
        var location = verify['location'] || null;
        ...
        verify['method'] = verify['method'] || "blur";

        var ver = new Verify(el, verify.megs, verify.name, verify.location);
        el.bind(verify['method'],function(){
    ...
    return {
         ...
         * @param fun  function 插件方法
         * @param megs string 错误提示
         *
         * */
        fn:function(name,fun,megs){
            Verify.prototype[name] = fun;
            Verify.prototype.megsp[name] = megs || "";
        },

        /**
         * 验证单个表单
         *
         * @param elem string元素ID或者选择器
         * @param verify  object 验证参数集合
         * {
         *
         * name string没有自定义错误消息时需要提示的名称。
         *
         * method string触发事件，默认blur
         *
         * check array 需要验证的方法集合
         *
         * location Element  提示显示的位置,可以为空, 默认是elem内自己设置的如果没有就是元素自己后面
         *
         *  megs object 可选参数，消息集合,key为验证的方法名， 可自定义，可直接输出，也可预定义
         * }
         *
         * */
        verifyInput:function(elem,verify){
        ...
        /**
         * 验证form 人口
         * @param formId string 需要验证的formID
         * @param verify  object 表单元素集合
         * @param async object 异步提交,可选参数{success 提交后回调函数，before 提交前执行函数}
         * @param location string 默认提示显示的位置,可以为空, 默认是verify内自己设置的如果没有就是元素自己后面
         * @param method string 触发方法,默认blur默认可选参数，
         * */
        verifyForm:function (formId,verify,async,location ,method) {
            ...
            for(var es in  verify){
                ...
                if(typeof verify[es]['location'] ==='undefined'){
                    verify[es]['location'] =  location;
                }
                if(typeof verify[es]['method'] ==='undefined'){
                    verify[es]['method'] =  method;
                }
                run(el, verify[es]);
            ...
            form.submit(function () {
                ...
                if (async) {
                    ...
                    //提交前执行回调
                    if (typeof async['before'] !== 'undefined' && async['before']() === false) {
                        return false;
                    }
                    $.ajax({
